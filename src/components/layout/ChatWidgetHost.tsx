...
function WhatsAppFab() {
  ...
  // Hint bubble: appears every 6s for 3s, stops after user has been on page 60s
  const [showHint, setShowHint] = useState(false);

  useEffect(() => {
    const HINT_VISIBLE_MS = 3000;   // bubble visible for 3s
    const HINT_INTERVAL_MS = 6000;  // every 6s
    const STOP_AFTER_MS = 60000;    // stop after 1 minute on page
    ...
    const interval = setInterval(cycle, HINT_INTERVAL_MS);
  ...
  return (
    ...
    >
      <span className="relative block">
        {/* Hint bubble — pulses in/out every 6s, stops after 1 min on page */}
        {showHint && (
          <span
        ...
        <span
          className="flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg ring-4 ring-[#25D366]/20"
